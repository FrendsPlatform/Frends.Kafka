...
// Ignoring null warnings because these methods are copied from Frends.Kafka.Producer and some refs might be unused. There will be exceptions if something is truly missing.
...
internal class MessageProducer
{
    internal static async Task ProduceBasic(Input input, Options options, Socket socket, Sasl sasl, Ssl ssl, string? msgKey, string? msg, CancellationToken cancellationToken)
    {
        ...
        if (topicPartition != null)
            ...
            await producer.ProduceAsync(input.Topic, message, cancellationToken);
    }
